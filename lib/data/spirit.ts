...
export async function getGratitudeEntries(): Promise<GratitudeEntry[]> {
  try {
    ...
    if (dbEntries.length === 0) return initialGratitudeEntries
    ...
    return initialGratitudeEntries
  }
}
...
export async function getDreamEntries(): Promise<DreamEntry[]> {
  try {
    ...
    if (dbEntries.length === 0) return initialDreamEntries
