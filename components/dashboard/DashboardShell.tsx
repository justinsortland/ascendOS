...
import { mockCommandCards, mockAIMessages, mockUserStats } from '@/lib/mock-data'
import { calculateMomentumScore, calculateXPEarned } from '@/lib/scoring'
import { toggleTask as dbToggleTask } from '@/lib/actions/tasks'
import type { Task, BonusQuest, DailyMission } from '@/lib/types'
...
interface Props {
  initialTasks: Task[]
  initialQuests: BonusQuest[]
  initialMission: DailyMission
  initialStats: typeof mockUserStats
}

export function DashboardShell({ initialTasks, initialQuests, initialMission, initialStats }: Props) {
  const [tasks, setTasks] = useState<Task[]>(initialTasks)
  const [mission, setMission] = useState(initialMission)
  function toggleTask(id: string) {
    setTasks(prev => prev.map(t => (t.id === id ? { ...t, completed: !t.completed } : t)))
    const task = tasks.find(t => t.id === id)
    if (task) dbToggleTask(id, !task.completed).catch(console.error)
  }
  ...
  const stats = { ...initialStats, xp: initialStats.xp + xpEarned, momentumScore: momentum }
  ...
  return (
    <div className="px-6 py-6 max-w-6xl">
      ...
      <div className="grid grid-cols-2 gap-4">
        <BonusQuestsCard quests={initialQuests} />
        <PomodoroCard />
      </div>
    </div>
  )
