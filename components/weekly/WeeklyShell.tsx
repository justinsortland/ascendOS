...
import { updateWeeklyTheme, saveWeeklyReview } from '@/lib/actions/weekly'
import type {
  WeeklyGoal,
  WeeklyMealPrepIdea,
  GroceryItem,
...
interface Props {
  ...
  initialMainFocus: string
  initialReviews: WeeklyReviewEntry[]
  dataSource: 'db' | 'mock'
}

export function WeeklyShell({ initialTheme, initialWeekRange, initialMainFocus, initialReviews, dataSource }: Props) {
  ...
  const [weeklyReviews, setWeeklyReviews] = useState<WeeklyReviewEntry[]>(initialReviews)

  async function handleThemeChange(newTheme: string) {
    setTheme(newTheme)
    if (dataSource === 'mock') {
      console.warn('[WeeklyShell] Mock mode — theme change is session-local only.')
      return
    }
    await updateWeeklyTheme(newTheme)
  }

  async function handleSaveReview(entry: WeeklyReviewEntry) {
    setWeeklyReviews(prev => [entry, ...prev])
    if (dataSource === 'mock') {
      console.warn('[WeeklyShell] Mock mode — review is session-local only.')
      return
    }
  ...
  return (
    <div className="px-6 py-6 max-w-6xl">
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <WeeklyThemeSelector theme={theme} onThemeChange={handleThemeChange} />
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <WeeklyReview entries={weeklyReviews} onSave={handleSaveReview} />
